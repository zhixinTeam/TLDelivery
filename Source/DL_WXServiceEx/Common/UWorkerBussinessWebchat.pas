...
var nStr, nType, nTypeCode: string;
    nNode: TXmlNode;
    nValue,nMoney, nMoneyTotal, nPrice: Double;
    nDBWorker: PDBWorker;
begin
  Result := False;
  BuildDefaultXML;
  ...
  try
    ...
    begin
      ...
      while not Eof do
      begin
        nMoney := GetCustomerValidMoney(FieldByName('contractCode').AsString);
        //获取合同可用金
        ...
        begin
          ...
          {$ELSE}
           ...
           NodeNew('StockType').ValueAsString := nTypeCode;
           NodeNew('StockName').ValueAsString  := FieldByName('prodName').AsString;
          {$ENDIF}

          {$IFDEF YHTL}
          NodeNew('UnitPrice').ValueAsString := FieldByName('salePrice').AsString;
          {$ENDIF}
...
begin
  ...
  try
    ...
    begin
      ...
      with nNode do
      begin
        NodeNew('ProvId').ValueAsString := FieldByName('supId').AsString;
        NodeNew('ProvName').ValueAsString := FieldByName('supName').AsString;
      end;
      ...
      begin
        with nNode.NodeNew('Item') do
        begin
          NodeNew('SetDate').ValueAsString    := DateTime2Str(FieldByName('makeDate').AsDateTime);
          NodeNew('BillNumber').ValueAsString := FieldByName('billNum').AsString;
          NodeNew('StockNo').ValueAsString    := FieldByName('itemCode').AsString;
          NodeNew('StockName').ValueAsString  := FieldByName('itemName').AsString;
          NodeNew('MaxNumber').ValueAsString  := FieldByName('itemQty').AsString;
          {$IFDEF KuangFa}
