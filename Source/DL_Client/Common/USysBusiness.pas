...
var nStr: string;
begin
  nStr := 'select * from sal.SAL_Contract_v where contractCode=''$ID''';
  nStr := MacroValue(nStr, [MI('$ID', nZID)]);
  //xxxxx
  ...
  with nList.Items,Result do
  begin
    Add('纸卡编号:' + nList.Delimiter + FieldByName('contractCode').AsString);
    Add('客户编号:' + nList.Delimiter + FieldByName('accountCode').AsString+ ' ');
    Add('客户名称:' + nList.Delimiter + FieldByName('accountName').AsString + ' ');
//    Add('项目名称:' + nList.Delimiter + FieldByName('Z_Project').AsString + ' ');

    nStr := DateTime2Str(FieldByName('validDate').AsDateTime);
    Add('生效时间:' + nList.Delimiter + nStr);

    nStr := DateTime2Str(FieldByName('invalidDate').AsDateTime);
    Add('失效时间:' + nList.Delimiter + nStr);
...
begin
  ...
  if Pos('fmhgsy', Result) > 0 then
    Result := gPath + 'HuaYan_FMHGSY.fr3'      //粉煤灰硅酸盐水泥
  else if Pos('gjgklsyyj', Result) > 0 then
    Result := gPath + 'HuaYan_YJ.fr3'          //G级高抗硫酸盐油井
  else if Pos('gklsygsy', Result) > 0 then
    Result := gPath + 'HuaYan_GKLSY.fr3'       //42.5高抗硫酸盐硅酸盐水泥
  else if Pos('ptyjsn', Result) > 0 then
    Result := gPath + 'HuaYan_PTYJ.fr3'        //普通油井水泥
  else if Pos('zklsygsy', Result) > 0 then
    Result := gPath + 'HuaYan_ZKLSY.fr3'       //中抗硫酸盐硅酸盐
  else if Pos('7.5dlgsy', Result) > 0 then
    Result := gPath + 'HuaYan_75.fr3'          //7.5道路硅酸盐水泥
  else if Pos('7.5dlgsy', Result) > 0 then
    Result := gPath + 'HuaYan_JC.fr3'          //42.5道路硅酸盐水泥
  else if Pos('shn', Result) > 0 then
    Result := gPath  + 'HuaYan_HN.fr3'         //散缓凝
