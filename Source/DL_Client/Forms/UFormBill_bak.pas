...
begin
  ...
  with gInfo do
  begin
    FCusID := nDB.FieldByName('accountcode').AsString;
    FPriceChanged := False;//nDB.FieldByName('Z_TJStatus').AsString = sFlag_TJOver;
    
    SetCtrlData(EditLading, nDB.FieldByName('transtyleName').AsString);
  ...
  nStr := 'Select * From %s Where contractCode=''%s''';
  nStr := Format(nStr, ['SAL.SAL_CTRItem', gInfo.FZhiKa]);
