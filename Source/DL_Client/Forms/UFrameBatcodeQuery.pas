...
uses
  ...
  ComCtrls, ToolWin, UFormCtrl, cxCheckBox, dxSkinsCore,
...
begin
  ...
  begin
    if SQLQuery.FieldByName('D_Valid').AsString = sPopedom_Delete then
    begin
      nStr := '禁止修改已删除批次号';
      ShowMsg(nStr, sHint);
      Exit;
...
begin
  inherited;
  nStr := '';
  if ckDel.Checked then nStr := Format('D_Valid=''%s''', [sPopedom_Delete]);
